# predict_molecule

OMol25 大规模分子力场模型。FieldQuanta 公司项目之一。

- 仓库: `~/predict_molecule` (本机, crystal 分支, 暂未推 GitHub)
- 当前里程碑: Matbench-Discovery 晶体稳定性 F1 (非等变 transformer + 数据/增广)
- 目标: F1 ≥ 0.9 (北极星 eSEN-30M-OAM 0.925; 同级参考 eSEN-30M-MPtrj 0.819)

## 子目录

- `daily/` — 每日日报
- `weekly/` — 周报 (ISO week, 每周二写)
- `notes/` — 长篇技术调研 / 决策札记

## 约定

- 加班当天的日报,文件名在日期后加 `(+)`,如 `2026-06-06(+).md`。

### 周报

- 每周二写,覆盖上周三~本周二,文件名 `YYYY-Www.md` (按当周二的 ISO week 编号)。
- **只写 2-3 点最关键进展**,不堆细节。
- **格式严格遵循最早的周报 `2026-W21` 的 3 列表格**:`| 主要工作内容 | 进展 | 可能存在的风险 |`(`## 主要工作` 标题 + 表格 + `## 下周 todo`)。
- **关键指标 highlight(加粗)**,如 **unique F1 0.8224**。
- **要外行也能看懂**:关键结论 + 数字用平实语言点出含义(F1 是什么、对标模型是什么、这个进展意味着什么),必要时给术语补一句解释,不假设读者懂背景。
- **技术术语可以用,内部黑话/隐喻不用**:CPS / µP / moonshot / coord-check / skip / rollback 等技术术语属正常表述,首次出现点一句含义即可;但"北极星""踩雷"这类内部隐喻/口头黑话不写进周报,换成平实说法(如"北极星"→"对标榜首模型")。
- 对标模型写全称精确版本 (如 `eSEN-30M-MPtrj` 0.819 / `eSEN-30M-OAM` 0.925),不简写成 "eSEN"。
